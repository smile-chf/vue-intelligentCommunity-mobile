<template>
  <div class="moodrealize">
    ...
    <div class="centeritem" v-if="actievstate">
      <div :class="['bottomline',item.showchange? 'notclick':'']" v-for="(item, index) in datalist" :key="index" @click="gofillin(item.id)">
    ...
    <!-- 已提交 -->
    <div class="centeritem" v-if="!actievstate">
      <div :class="['bottomline',item.showchange? '':'']" v-for="(item, index) in datalist" :key="index" @click="gonextpage(item.id)">
        <div  :class="['cleft',item.showchange? 'end':'']">
           <div class="yeardata">{{item.conferenceDate.slice("0","7")}}</div>
          <div class="daydata">{{item.conferenceDate.slice("8","10")}}</div>
          <div class="weekdata">{{item.conferenceDate.slice("10")}}</div>
...
<script>
import {conferences} from "../../api/http"
export default {
  data() {
    return {
      actievstate: true,
  ...
  methods: {
    ...
    gonextpage(id) {
      this.$router.push({ path: "/detailedmetting",query:{id,form:"meetingrecord"} });
    },
    async getconferences(val){
      let res =await conferences(val)
      console.log(res);
