...
<script>
import {experienceConferences} from "../../api/http"
export default {
  data() {
    return {
      status:1,
  ...
  methods: {
    ...
    gofillin(id) {
      ...
      this.$router.push({ path: "/fillinmoodrealize",query:{id} });
    },
    gosomepage(id) {
      this.$router.push({ path: "/detailedmetting",query:{id,form:'moodrealize'} });
    },
   async getexperienceConferences (){
        let res =await experienceConferences (this.status)
        // console.log(res);
