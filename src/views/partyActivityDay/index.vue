  ...
  <div class="threemeeting">
    <div class="threemeetingbottom">
      <van-grid column-num="4" icon-size="35">
        <van-grid-item :icon="fqhyurl"  text="发起活动日" @click="gosponsormeeting"/>
        <van-grid-item :icon="xdthurl" to="/activityDaySummarize" text="活动总结" />
        <!-- <van-grid-item :icon="hyjlurl" to="/meetingrecord" text="会议纪要" /> -->
        <van-grid-item :icon="qjurl"  to="/activityDayLeave" text="请假" />
        <van-grid-item :icon="wdhyurl" to="/myActivityDay" text="我的活动日" />
      </van-grid>
    </div>
...
<script>
import {EventDayfourconferences,getMemberIsAdministrator} from '@/api/http'
export default {
  data() {
    return {
      fqhyurl: require("@/img/someimg/ico3.png"),
      xdthurl: require("@/img/someimg/ico1.png"),
      qjurl: require("@/img/someimg/ico2.png"),
      wdhyurl: require("@/img/someimg/ico4.png"),
      datalist:[]
    };
  },
  ...
  async  gosponsormeeting(){
      ...
      if (show=="是") {
          this.$router.push("/addActivityDaay")
      } else {
        this.$toast.fail({ message: "该用户无此权限", duration: 3000 });
      }
    },
     async getfourconferences(val) {
      let res = await EventDayfourconferences(val);
      console.log(res);
